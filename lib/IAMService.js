...
class IAMService {
  ...
  getTideCloakClient() {
    if (!_tc) {
      if (Object.keys(kcData).length == 0) {
        console.log(
          "[getTideCloakClient] tidecloak.json is empty. Web admin must download client adaptor from TideCloak or initialise on Login screen."
        ); // Thrown to catch
      } else {
        _tc = new TideCloak({
          url: kcData["auth-server-url"],
   ...
   */
  initIAM(onReady) {
    const tidecloak = this.getTideCloakClient();
